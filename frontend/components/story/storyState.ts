...
export type CreateLocalStoryInput = {
  authorId: string;
  authorName: string;
  authorInitials: string;
  avatarUrl: string | null;
...
export function resolveStoryUrl(url: string | null | undefined, apiUrl = DEFAULT_API_URL): string | null {
  if (!url) return null;
